...
exports.create = function(request, response) {
	...
	var guidInstance = guid.guid();
    	try 
	{
        	if((request.body.fullName != null && request.body.fullName.length != 0) && (request.body.email != null && request.body.email.length != 0) && (request.body.phone != null && request.body.phone.length != 0) && (request.body.authTypeId != null && request.body.authTypeId.length != 0)) 
		{
            		if(request.body.phone != null) 
			{
                		...
                		if(!request.body.phone.match(phoneFilter))
                		{
                    			...
                    			log.info({Function: "User.Create, ID: " + guidInstance}, "Not a valid phone number. Phone:" + request.body.phone + "ID: " + guidInstance);
			...
			{
                    		log.error(connectionError + "ID: " + guidInstance, "Database connection error (Function = User.Create)");
                    		json = {
                        		ERROR: "User Create failed. Database could not be reached. ID: " + guidInstance
                    		};
                	...
                	connection.query('SELECT id FROM '+ config.mysql.db.name +'.user WHERE phone = ?', [request.body.phone], function(queryError, result) {
                    	if(queryError != null) 
			{
                        	log.error(queryError + "ID : " + guidInstance, "Query error. Failed to create a new user. User Details: " + request.body.phone + "(Function = User.Create)");
                        	json = {
                            		ERROR: "Requested action failed. Database could not be reached. ID: " + guidInstance
                        	};
                    	...
                    	if (result[0]) 
			{
				...
				connection.query('CALL ' + config.mysql.db.name + 'createSessionToken(?, ?, @outSessionToken, @outStatus, @outExpiresAt', userId,[request.body.authTypeId], function(sessionQueryError, sessionResult) {
				if(sessionQueryError != null)
				{
					log.error(sessionQueryError + "ID: " + guidInstance, "Session Query error. Failed to create a new user. User Details: " + JSON.stringify(request.body.phone) + " (Function = User.Create)");
                        	}
				console.log("Session Result: " + JSON.stringify(sessionResult));
				else (sessionResult[0]) {
                            	var jsnData = {};
                            	jsnData['sessionToken'] = sessionResult[0].sessionToken;
                            	jsnData['expiresAt'] = sessionResult[0].expiry_date;
